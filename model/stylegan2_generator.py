...
def build(model_name, use_cuda=False):
    ...
    generator = StyleGAN2Generator(model_config['resolution'], use_cuda=use_cuda)
...
class StyleGAN2Generator(nn.Module):
    ...
    def forward(self,
        ...
        # Generate from W space.
            ...
            wp, wp_no_trunc = self.truncation(ws, trunc_psi, trunc_layers)
            results['w'] = ws
            # results['wp'] = self.get_value(wp)
            results['wp'] = wp
            results['wp_no_trunc'] = wp_no_trunc
        ...
        # Generate from W+ space.
            ...
            wp, wp_no_trunc = self.truncation(wp, trunc_psi, trunc_layers)
            results['wp'] = self.get_value(wp)
            results['wp_no_trunc'] = wp_no_trunc
